...
with open(fileName, 'w') as file:
    for elem in rows:
        ...
        query = 'SELECT date FROM samples WHERE sample_id IN (SELECT sample_id FROM specimen WHERE species_id= %s)'
        cursor.execute(query, (elem))
        ...
        file.write(str(sorted(dateList)) + '\n')
        print(sorted(dateList),type(dateList))
        descript = set()
        for stand in stands:
